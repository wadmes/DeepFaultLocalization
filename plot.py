...
def main():  
    ...
    mfr_y = final_result[:,len(subs),m,3]
    ...
    maximal_top1_x = np.argwhere(top1_y == np.amax(top1_y)).flatten()
    
    for i in maximal_top1_x:
        step = x[i]
        top1_this_step = top1_y[i]
        mfr_this_step = mfr_y[i]
        print(step,top1_this_step,mfr_this_step )
